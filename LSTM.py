...
#apply padding
padded_text = pad_sequences(sequences, padding='post', maxlen=10)
print(vocab_size)
print(padded_text)

#create LSTM
model = Sequential()
model.add(Embedding(input_dim=vocab_size+1, mask_zero=True, output_dim=100))
...
#fit model
score = model.fit(padded_text, speech_acts, epochs=2)
